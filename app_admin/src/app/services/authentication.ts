...
export class Authentication {
  ...
  // the key for our token 'destination-token'
  public getToken(): string {
    let out: any;
    out = this.storage.getItem('destination-token');
  ...
  public saveToken(token: string): void {
    this.storage.setItem('destination-token', token);
  }

  // Logout of our application and remove the JWT from Storage
  public logout(): void {
    this.storage.removeItem('destination-token');
  }
  ...
  // Login method that leverages the login method in destinationDataService
  ...
  public login(user: User, passwd: string) : void {
    this.destinationData.login(user,passwd)
      .subscribe({
        next: (value: any) => {
          if(value)
          {
  ...
  public register(user: User, passwd: string) : void {
    this.destinationData.register(user,passwd)
      .subscribe({
        next: (value: any) => {
          if(value)
          {
